...
{
    public class ExtractFileGenerationService
    {
        ...
        public ExtractFileGenerationService(ILogger logger, IConfiguration configuration)
        {
            this._logger = logger;
        ...
        public async Task CreatePowerExtractFile(DateTime extractDate, IDictionary<DateTime, double> data)
        {
            try
            {
            ...
            {
                await _logger.LogError($"Error in {nameof(ExtractFileGenerationService.CreatePowerExtractFile)}", ex);
            }
        }
