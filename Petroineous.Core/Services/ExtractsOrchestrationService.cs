...
using Petroineous.Core.Configuration;
...
{
    public class ExtractsOrchestrationService
    {
        private readonly IEventAggregator _eventAggregator;
        private readonly IPositionDataService _positionDataService;
        private readonly IExtractFileGenerationService _extractFileService;
        ...
        public ExtractsOrchestrationService(IEventAggregator eventAggregator,
            ...
            IExtractFileGenerationService extractFileGenerationService,
        ...
        public async Task Start()
        {
            try
            {
                ...
                _powerExtractScheduler = _eventAggregator
                                            .ObserveInterval(TimeSpan.FromMinutes(powerExtractInterval), Constants.ExtractType.IntradayPowerPosition)
        ...
        private async Task RunExtract(string extractName, DateTime runTimestamp)
        {
            try
            {
                ...
                throw new OperationCanceledException($"This extract type is not yet supported. Extract Type =  {extractName}");
        ...
        private async Task RunPowerGenerationIntradayExtract(DateTime runTimestamp)
        {
            ...
            try
            {
                ...
                await _extractFileService.CreatePowerExtractFile(runTimestamp.Date, data);
